...
- Overridable keys:
	...
	- `rule`:
		...
		Will be used when [thematic breaks](#thematic-breaks) (`---` in Markdown) are encountered.
		Defaults to `line.with(length: 100%)`.
		If you are targeting HTML,
		you may want to set this to `html.hr`.
...
```typ
#cmarker.render(read("yourfile.md"), scope: (rule: html.hr))
```
