...
- `heading-label-case`:
	...
	- Accepted values:
		- `"kebab"`:
			Strip out invalid characters, lowercase everything, and convert ASCII spaces to hyphens.
			For example, `# My heading` will become `<my-heading>`.
			This is approximately consistent with
			[GitHub’s slugification algorithm](https://github.com/Flet/github-slugger).
		- `"kebab-preserve"`:
			As above, but don’t lowercase.
			This is more similar to Jupyter’s convention.
			For example, `# My heading` will become `<My-heading>`.
	- Default value: `"kebab"`.
