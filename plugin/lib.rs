...
type HtmlTags<'input> = HashMap<CaseInsensitiveKey<&'input [u8]>, HtmlTagKind>;
...
fn render(markdown: &[u8], options: &[u8]) -> Result<Vec<u8>, String> {
    ...
    let mut html_tags: HtmlTags<'_> = HashMap::new();
    for data in html_tags_bytes.split_inclusive(|&n| n >= 0xFC) {
        let (kind, tag) = data.split_last().unwrap();
        let kind = match kind {
            0xFC => HtmlTagKind::Void,
            0xFD => HtmlTagKind::RawText,
            0xFE => HtmlTagKind::EscapableRawText,
            0xFF => HtmlTagKind::Normal,
            _ => panic!(),
        };
        html_tags.insert(CaseInsensitive(CaseInsensitive(tag)), kind);
    }

    inner(markdown, &html_tags, options, h1_level)
}
...
type CaseInsensitiveKey<T> = CaseInsensitive<CaseInsensitive<T>>;
...
mod tests {
    ...
    fn render(s: &str, html_tags: &HtmlTags<'_>, options: Options, h1_level: u8) -> String {
        String::from_utf8(super::inner(s, html_tags, options, h1_level).unwrap()).unwrap()
    }
